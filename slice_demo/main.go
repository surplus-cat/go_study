...
func main()  {
	...
	var intArr [5]int = [...]int{1, 2, 3, 44, 66};
	slice := intArr[1:3];
	//slice就是切片名
	...
	var slice3 []string = []string{"tim", "weixin", "tony"};
	...
	var arr [5]int = [...]int{10, 30, 100, 300, 1000}
	slice4 := arr[1:4]
	for i:= 0; i < len(slice4); i++ {
		fmt.Println("slice4[%v]=%v\n", i, slice4[i])
	}
	...
	var slice6 []int = []int{1000, 2000, 3000}
	slice6 = append(slice6, 4000, 6000);
	...
	var slice7 []int = []int{1, 3, 45, 55, 6};
	...
	var a []int = []int{1, 2, 3, 10, 6};
	var slice9 = make([]int, 6);
	fmt.Println(slice9);
	copy(slice9, a);
	fmt.Println(slice9);
